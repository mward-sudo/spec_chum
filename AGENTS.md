...
2. If missing/pending/on-demand-skip: first pass → `@coderabbitai full review` (or label); after prior full review + fixes → `@coderabbitai review`. Prefer both sides when available. If rate-limited after a request: do not burn quota; either-side fallback (local clean **or** wait for GitHub completed); optional revisit issue. If both stay rate-limited >10m: merge with revisit issue + PR note (standing policy).
