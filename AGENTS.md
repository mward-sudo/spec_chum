...
Any task to **finish, land, or merge a PR** must include this gate. **Either-side CodeRabbit:** only one of local clean CR **or** GitHub `Review completed` on HEAD is required (after GitHub was requested, or rate-limited after a request). A **ready** PR is **not merge-ready** while **either**:
...
unless the user **explicitly** waives (`rmw` / `rmcw`). **Rate-limited after a request** (including green `Review rate limited`) is **not** “Review completed” and is **not** a hard merge hold when the other side completed cleanly: gate 1 soft-passes with a warning; disposition threads; optional revisit issue (e.g. [#181](https://github.com/mward-sudo/spec_chum/issues/181)). Soft-pass ≠ skip-without-request. **Dual rate-limit (>10m):** if both local and GitHub stay rate-limited, merge under standing policy with a revisit issue (e.g. [#289](https://github.com/mward-sudo/spec_chum/issues/289)) documented on the PR — do not wait forever. **Drafts** may skip CR completeness in the script but still fail on unresolved bot threads; do not merge drafts. User shorthand: `rm` / `rmc` = merge, no extra waiver for normal soft-pass; `rmw` / `rmcw` = one-shot waiver for other holds.
...
2. If missing/pending/on-demand-skip: first pass → `@coderabbitai full review` (or label); after prior full review + fixes → `@coderabbitai review`. If rate-limited after a request: do not burn quota; rely on either-side (local clean **or** wait for GitHub completed); optional revisit issue. If both stay rate-limited >10m: merge with revisit issue + PR note (standing policy).
...
4. User-explicit waiver (`rmw` / `rmcw`, `--waive`, `SPEC_CHUM_REVIEW_WAIVER`, or label `waive-bot-reviews`) for holds beyond normal soft-pass (including unresolved bot threads) — document on the PR. Either-side rate-limit soft-pass and dual >10m standing policy do not require that waiver; on-demand skip does not soft-pass.
