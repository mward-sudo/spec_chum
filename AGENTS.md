...
2. If missing/pending/on-demand-skip: first pass → `@coderabbitai full review` (or label); after prior full review + fixes → `@coderabbitai review`. If rate-limited after a request: do not burn quota; rely on local CR + soft-pass; optional revisit issue. Waiver only if both local and GitHub CR stay rate-limited for >10m (user-approved).
...
4. Waiver only with user instruction: `--waive`, `SPEC_CHUM_REVIEW_WAIVER`, or label `waive-bot-reviews` — document on the PR. (Rate-limit soft-pass does not require this waiver; on-demand skip does not soft-pass.)
