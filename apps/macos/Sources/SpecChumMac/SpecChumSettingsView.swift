        ...
        Form {
            Section("Machine") {
                ...
                Text("Built-ins use default ROMs from ./scripts/fetch_roms.sh. Custom ROM overrides belong in a saved configuration only.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                Text("Timex TC2048 / TS2068: alt display file, hi-colour (8×1), and 512×192 hi-res (modes 4–7) are drawn. See docs/TIMEX.md.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            Section("Tape") {
                ...
                .help("While Play: N Spectrum frames per tick (wall-clock ≈ realtime/N). Applies to loading only — once the deck finishes the loaded program runs at 1×. Instant flash ignores this.")
