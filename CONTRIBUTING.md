...
- **Soft-pass gate 1 (CI) / merge rules:** `Review rate limited` (or similar *after* a request; often green) — **not** `Review completed`. Script warns and continues (CI cannot see local CR). **Merge** still requires **either-completed** (clean local CR, or later `Review completed`) **or** **dual** rate-limit: **both** local and GitHub rate-limited with resets **>10m** (or long/unknown on both) + revisit issue (e.g. [#181](https://github.com/mward-sudo/spec_chum/issues/181)). If **either** side reports **≤10m** (or can review now), wait/retry — do not merge on unilateral GitHub rate-limit.
- **Rate-limit wait vs dual soft-pass (reported reset):** If **either** side can review now or reports reset **≤10 minutes** → **wait/retry that side** (do not soft-pass-merge yet). Dual soft-pass merge (no completed CR) only when **both** local and GitHub are rate-limited **and** resets are **>10 minutes** (or long/unknown on both): open/link a revisit issue (e.g. [#289](https://github.com/mward-sudo/spec_chum/issues/289), [#305](https://github.com/mward-sudo/spec_chum/issues/305)), document on the PR, then merge. User may already declare both sides >10m → merge immediately (e.g. [#304](https://github.com/mward-sudo/spec_chum/pull/304)). Do **not** sleep a 40–50m dual reset. Scripts soft-pass GitHub rate-limit in CI only; they do not parse the 10-minute timer (advisory / agent process).
