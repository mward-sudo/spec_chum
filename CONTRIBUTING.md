...
- **Before merge / finish PR:** mark ready if still draft; **prefer both** — run local CR **and** request GitHub CodeRabbit (`@coderabbitai full review` or label) when merge-candidate if not rate-limited; **either** local clean **or** GitHub `Review completed` is enough if only one can finish; then run `./scripts/check_pr_reviews.sh`. **Hold** on pending/missing/error CodeRabbit, **on-demand / label skips** (review never requested), or unresolved **actionable** bot threads unless the user explicitly waives (`rmw` / `rmcw`). **Rate-limited** soft-pass / either-side is the fallback when one side cannot complete. **Wait rule:** reported reset **< 10m** → OK to wait and retry; reset **≥ 10m** (or both sides long/unknown) → dual merge **immediately** (revisit issue + PR note) — do not sleep long resets. Soft-pass ≠ skip-without-request. See below.
...
- **Soft-pass / either-side gate 1 (fallback):** `Review rate limited` (or similar quota unavailability *after* a review was requested; often green) — **not** treated as `Review completed`. Script warns loudly and continues. Prefer both sides when available; fallback is a **clean local** CR (Cursor plugin / `coderabbit review --agent`) with dispositioned findings, or GitHub `Review completed` if that side finished. Soft-pass on one side is fine when the other completed. Optional revisit issue (e.g. [#181](https://github.com/mward-sudo/spec_chum/issues/181)) when quota resets — not a merge hold for single-side soft-pass.
- **Rate-limit wait vs dual merge (reported reset):** If CR reports reset **< 10 minutes** → OK to wait that duration and retry. If reset **≥ 10 minutes** (or both sides rate-limited with long/unknown reset) → **do NOT wait** — dual merge **immediately** under standing policy: open/link a revisit issue (e.g. [#289](https://github.com/mward-sudo/spec_chum/issues/289), [#305](https://github.com/mward-sudo/spec_chum/issues/305)), document on the PR, then merge. User may already declare both sides ≥10m → merge immediately (e.g. [#304](https://github.com/mward-sudo/spec_chum/pull/304)). Do **not** sleep a 40–50m reset. Scripts do not enforce the 10-minute threshold (advisory / docs-only).
