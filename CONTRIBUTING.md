...
This **complements** the GitHub PR workflow below. Ready PRs should **prefer both** local CR and GitHub `@coderabbitai full review` / `@coderabbitai review` when available (or label `coderabbit-review`), plus `./scripts/check_pr_reviews.sh` before merge. **Either completed** side is enough: clean local CR **or** GitHub `Review completed` on HEAD. When GitHub is **rate-limited after a request**, run local CR if available (or wait if local reset **≤10m**); dual soft-pass merge only if **both** sides are rate-limited **>10m** (+ revisit issue; `rm` / `rmc` needs no extra waiver). On-demand / label skips hard-fail — request a review.
...
- **Soft-pass gate 1 (CI) / merge rules:** `Review rate limited` (or similar *after* a request; often green) — **not** `Review completed`. CI soft-passes when reset is **>10m** (parsed from status description such as “Next included review available in N minutes”) **or** unparsable (loud warning; prefer soft-pass over brittle false reds); **HOLDs** when reset is reliably parsed as **≤10m**. CI cannot see local CR. **Merge** still requires **either-completed** (clean local CR, or later `Review completed`) **or** **dual** rate-limit: **both** local and GitHub rate-limited with resets **>10m** (or long/unknown on both) + revisit issue (e.g. [#181](https://github.com/mward-sudo/spec_chum/issues/181)). If **either** side reports **≤10m** (or can review now), wait/retry — do not merge on unilateral GitHub rate-limit.
- **Rate-limit wait vs dual soft-pass (reported reset):** If **either** side can review now or reports reset **≤10 minutes** → **wait/retry that side** (do not soft-pass-merge yet; CI also HOLDs when GitHub’s description parses to ≤10m). Dual soft-pass merge (no completed CR) only when **both** local and GitHub are rate-limited **and** resets are **>10 minutes** (or long/unknown on both): open/link a revisit issue (e.g. [#289](https://github.com/mward-sudo/spec_chum/issues/289), [#305](https://github.com/mward-sudo/spec_chum/issues/305)), document on the PR, then merge. User may already declare both sides >10m → merge immediately (e.g. [#304](https://github.com/mward-sudo/spec_chum/pull/304)). Do **not** sleep a 40–50m dual reset.
- **Hard-fail gate 2:** unresolved actionable bot review threads (unless user waives via `rmw` / `rmcw`, `--waive`, or label). Either-completed and dual (>10m both sides) need no waiver (`rm` / `rmc`).
...
**CI:** workflow **Bot review threads** (`.github/workflows/pr-bot-reviews.yml`) runs `./scripts/check_pr_reviews.sh` on PRs, when reviews/comments arrive, and when CodeRabbit updates its **commit status** (so a prior pending/in-progress red check clears on `Review completed` / terminal rate-limited without a manual re-run). Uses the default `GITHUB_TOKEN`. The ruleset required context is the API-published check **`unresolved bot threads`** (job name is `bot-review-gate` so cancelled runs cannot block merge — [#318](https://github.com/mward-sudo/spec_chum/issues/318)). Treat a failing **published** check as blocking for merge. After resolving threads, re-run that job if GitHub did not re-trigger it. On-demand/label skips hard-fail; GitHub rate-limited soft-passes CI when reset **>10m** or unparsable, HOLDs when parsed **≤10m** (agents must still confirm either-completed or dual >10m for merge; soft-pass ≠ skip).
...
The script (1) on ready PRs checks CodeRabbit on HEAD — hard-fails pending/missing/error/on-demand-skip and rate-limit with parsed reset ≤10m; **soft-passes** GitHub rate-limited when reset >10m or unparsable (stderr warning that merge still needs either-completed or dual >10m; drafts skip this step; CI cannot see local CR); then (2) paginates GraphQL `reviewThreads`, prints unresolved bot comment URLs, and exits non-zero unless waived. Cursor rule: `.cursor/rules/pr-review-merge.mdc`.
