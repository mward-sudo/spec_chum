...
_spec_chum_use_local_caches() {
  local reason="${1:-SSD not mounted — local caches (\$REPO/target, apps/macos/.build)}"
...
if _spec_chum_ssd_usable; then
  ...
  then
    _spec_chum_use_local_caches "failed to create SSD cache dirs — keeping local caches"
  else
...
unset -f _spec_chum_path_on_ssd _spec_chum_ssd_usable _spec_chum_use_local_caches 2>/dev/null || true
