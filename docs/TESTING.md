...
- `unwrap_used = warn`; library crates use `thiserror` (no bare unwrap on non-test hot paths). Bus ROM/EEPROM size checks return `bus::RomLoadError`; `Machine::new_*` returns `MachineBuildError` wrapping that type ([#171](https://github.com/mward-sudo/spec_chum/issues/171) Pillar B — further `Result<_, String>` sites remain).
