...
- Non-test code must not use bare `unwrap`. Workspace `unwrap_used = warn` is promoted to an error by CI and `./scripts/check.sh` through `-D warnings`. Prefer `?` or typed errors; library crates use `thiserror`. Typed errors cover ROM load/build (`bus::RomLoadError`, `MachineBuildError`, `RomReadError`), peripheral attach (`MultifaceError` / `DivMmcError` / `BetaDiskError` / `Interface1Error` / `InsertDiskError`), Fuse `FuseCaseError`, z80test `Z80testError`, living_room `HeadlessRoomError` / `PresentIosurfaceError`, egui `BuildMachineError`, and agent `ReadyError` ([#171](https://github.com/mward-sudo/spec_chum/issues/171) Pillar B). Remaining `Result<…, _>` → `String` **Ok** payloads are intentional text APIs (inspect / peek / disasm / trace dump), not error-typing gaps.
- `expect_used = allow`: use `expect` only for a documented invariant that cannot fail at that point, and make the message state that invariant. `expect` is not automatically safe merely because Clippy permits it; this restriction is a project rule.
...
| Interface 1 / Microdrive | [#139](https://github.com/mward-sudo/spec_chum/issues/139), [#397](https://github.com/mward-sudo/spec_chum/issues/397) | Fuse MDR layout + checksums; ROM-gated CAT/FORMAT soaks assert motor-off + `ERR_NR==OK` (and FORMAT `looks_formatted`); skip without `roms/if1.rom` |
| Multiface / DivMMC | [#138](https://github.com/mward-sudo/spec_chum/issues/138), [#168](https://github.com/mward-sudo/spec_chum/issues/168) | Multiface: ROM-gated soak. DivMMC: delayed automap + SPI stub always on; ESXDOS boot smoke skips without `roms/divmmc/ESXMMC.BIN` + `roms/divmmc/esxdos.img` ([docs/ROMS.md](ROMS.md)) |
