...
- **Hardware:** Multiface 1 attach + NMI (`sc_attach_multiface` / `sc_multiface_nmi`, 48K). Supply your own **8 KiB** Multiface ROM (not shipped — see [MULTIFACE.md](MULTIFACE.md)). Interface 1 attach + MDR (`sc_attach_interface1` / `sc_insert_mdr`); optional IF1 ROM via `sc_load_interface1_rom` or `roms/if1.rom`. DivMMC attach + SD/EEPROM (`sc_attach_divmmc` / `sc_load_divmmc_sd` / `sc_load_divmmc_eeprom`, 48K/128K). Joystick mode is **Settings → Input**. Beta Disk: Hardware → **Attach Beta Disk**, **Load TR-DOS ROM…**, **Open TRD…** plus File / toolbar **Open Tape / TRD** (`sc_attach_beta` / `sc_load_trdos_rom` / `sc_load_trd`) on 48K/128K — same workflow as egui.
