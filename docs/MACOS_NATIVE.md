...
- **Session prefs (#186):** model, EAR speed / Experience, joystick mode, Kempston mouse, and recent files also persist in `UserDefaults` (`specChum.model`, `specChum.tapeEarSpeed`, `specChum.tapeExperience`, `specChum.joystickMode`, `specChum.kempstonMouse`, `specChum.recentFiles`). Window frame uses `NSWindow` autosave (`SpecChumMainWindow`). Instant / flash-load is never sticky. egui / living_room share a JSON file (`ui-prefs.json`; override with `SPEC_CHUM_PREFS_PATH`)
