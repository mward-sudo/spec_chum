...
- **Hardware:** Multiface 1 attach + NMI (`sc_attach_multiface` / `sc_multiface_nmi`, 48K). Interface 1 attach + MDR (`sc_attach_interface1` / `sc_insert_mdr`); optional IF1 ROM via `sc_load_interface1_rom` or `roms/if1.rom`. Joystick mode is **Settings → Input**. DivMMC / Beta stubs remain **egui** Hardware first.
...
- DivMMC / Beta UI on macOS (egui Hardware stubs first); IF1 + MDR are in the native Hardware menu
