...
- **Hardware:** Multiface attach + NMI (`sc_attach_multiface` / `sc_multiface_nmi` — Multiface 1 on 48K-class, Multiface 128 on 128K/+2; +2A/+3 reject). Supply your own **8 KiB** Multiface ROM (not shipped — see [MULTIFACE.md](MULTIFACE.md)). Interface 1 attach + MDR (`sc_attach_interface1` / `sc_insert_mdr`); optional IF1 ROM via `sc_load_interface1_rom` or `roms/if1.rom`. DivMMC attach + SD/EEPROM (`sc_attach_divmmc` / `sc_load_divmmc_sd` / `sc_load_divmmc_sd_slot` / `sc_load_divmmc_eeprom`, 48K/128K) — Hardware menu opens slot 0 or slot 1 SD images. Joystick mode is **Settings → Input**. Beta Disk: Hardware → **Attach Beta Disk**, **Load TR-DOS ROM…**, **Open TRD…** plus File / toolbar **Open Tape / TRD** (`sc_attach_beta` / `sc_load_trdos_rom` / `sc_load_trd`) on 48K/128K — same workflow as egui.
...
4. **File → Open TRD…** (or toolbar **Open Tape / TRD**, Hardware → **Open TRD…**) — `.trd` on **48K/128K** (`sc_load_trd`, attaches Beta). **Hardware → Attach Beta Disk** with no media (`sc_attach_beta`). **Hardware → Load TR-DOS ROM…** for the 16 KiB ROM (`sc_load_trdos_rom`, not fetched by `fetch_roms.sh`). Enter TR-DOS with `RANDOMIZE USR 15616`.
...
- Windows / Linux native-shell strategy and decisions: [UI_ARCHITECTURE.md — Native shells](UI_ARCHITECTURE.md#native-shells-351) ([#351](https://github.com/mward-sudo/spec_chum/issues/351)). Linux ships egui as the product UI and has an optional GTK4 vertical slice — [LINUX_NATIVE.md](LINUX_NATIVE.md); Windows ships egui in releases and has an optional Win32 vertical slice — [WINDOWS_NATIVE.md](WINDOWS_NATIVE.md).
