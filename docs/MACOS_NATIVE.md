...
- **Hardware:** Multiface 1 attach + NMI (`sc_attach_multiface` / `sc_multiface_nmi`, 48K). Joystick mode is **Settings → Input**. File → **Open TRD…** and Hardware → **Load TR-DOS ROM…** (`sc_load_trd` / `sc_load_trdos_rom`) on 48K/128K. DivMMC / IF1 remain egui-first.
...
- DivMMC / IF1 UI on macOS (egui Hardware stubs first)
