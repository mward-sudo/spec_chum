...
| `trdos_rom_run_boot_basic_when_fixture_present` | **Open:** `RUN` → `POKE 32768,165` (`0x8000 == 0xA5`); fails with FDC histogram when `trdos.rom` present until TR-DOS init catalog completes (seek-only loop, PC≈`0xFE8E`) |
