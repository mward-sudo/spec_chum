...
| `trdos_native_file_services_gate_when_fixture_present` | Soft: hole dump → document FF at `08D2h`/`0D6Bh`; **classic** complete dump (filled hole, live `08D2h` file-load) → native RUN eligible |
| `trdos_19ec_takes_fdc_standin_when_504t_port_stub_present` | **5.04T VG93-stub** dump only (`LD A,#2C; JP 0897h` at `08D2h`): skips classic complete ROMs; `19ECh` still takes FDC/`LINE-NEW` stand-in |
| `trdos_rom_run_boot_504t_catalog_match_when_complete_present` | **5.04T VG93-stub** dump only: Type-II BUSY so `1E3Dh` returns to `1981h`; catalog match + `19ECh` stand-in → `0x8000==0xA5` (skips classic complete / hole dumps) |
...
Complete dumps are not interchangeable: a **classic** filled-hole image (live
`08D2h` file-load) enables native RUN; Alone Coder / VfNG **5.04T** fills the hole
with VG93 port remapping (`08D2h` = stub, not file-load) and still needs the
`19ECh` FDC stand-in. Prefer `roms/pentagon/trdos-5.04t.rom` or
`roms/pentagon/trdos-complete.rom` (also under `roms/trdos/`). Commercial `.trd`
titles stay local; do not add them to this tree.
