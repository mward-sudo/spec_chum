...
| `trdos_rom_run_boot_basic_when_fixture_present` | **Open:** `RUN` → `POKE 32768,165` (`0x8000 == 0xA5`); FDC `OUT #7F`/`IN #3F` seek compare fixed; RUN harness still open (`3D94` `RST #20` + `DJNZ` needs `B=0` before `CALL 3D80h`) |
