...
//!
//! Preferred entry from release builds: `spec_chum --serve` or `spec_chum debug …`
...
/// Run the headless debugger using process arguments (`std::env::args_os`).
pub fn run() -> Result<()> {
    run_from_args(std::env::args_os())
}

/// Run the headless debugger with an explicit argv (program name first).
...
#[command(
    name = "spec_chum",
    about = "Headless Spec Chum debugger / agent HTTP server",
    after_help = "Preferred: `spec_chum --serve …` or `spec_chum debug <args>`.\n\
...
mod tests {
    ...
    fn serve_without_subcommand() {
        let cli = Cli::try_parse_from(["spec_chum", "--serve", "--model", "48k"]).expect("parse");
        assert!(cli.serve);
