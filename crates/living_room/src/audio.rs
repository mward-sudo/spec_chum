...
/// Seconds of *speaker* PCM (post DC-block) to capture once live.
...
fn start_audio() -> (AudioOut, AudioStream) {
    ...
    let record = Arc::new(Mutex::new(Some(Vec::with_capacity(RECORD_SAMPLES))));
    ...
    let stream = match default_config.sample_format() {
        cpal::SampleFormat::F32 => device.build_output_stream(
            ...
            move |data: &mut [f32], _| {
                fill_output(
                    data, channels, &buf_cb, &last_cb, &live_cb, &fade_cb, &dc_cb, &rec_cb,
                );
            },
            err_fn,
            None,
        ...
        cpal::SampleFormat::I16 => device.build_output_stream(
            ...
            move |data: &mut [i16], _| {
                ...
                fill_output(
                    &mut tmp, channels, &buf_cb, &last_cb, &live_cb, &fade_cb, &dc_cb, &rec_cb,
                );
    ...
    match stream {
        Ok(stream) => {
            eprintln!(
                "living_room: audio stream {play_rate} Hz, {channels} ch (HostSession PCM @ {AUDIO_SAMPLE_RATE} Hz); DC-block on; recording first {RECORD_SECS}s speaker PCM → /tmp/spec-chum-room-10s.wav"
            );
            let _ = stream.play();
            (
                out,
        ...
        if let Some(cap) = rec.as_mut() {
            if cap.len() < RECORD_SAMPLES {
                ...
                if cap.len() >= RECORD_SAMPLES {
                    ...
                    if let Err(e) = write_wav_mono_f32(&path, AUDIO_SAMPLE_RATE, cap) {
