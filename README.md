...
- **+3DOS** — µPD765 command → execution → result (SPECIFY, SDS, SIS, RECALIBRATE, SEEK, READ ID, READ/WRITE DATA, FORMAT TRACK, write-protect). Loader talks to the FDC on a synthetic 180K DATA disk. Still unsupported: SCAN EQUAL/LOW/HIGH, READ TRACK, copy-protected / weird DSK geometry, commercial-title `RUN` (no copyrighted fixture yet; smoke list TBD) ([#141](https://github.com/mward-sudo/spec_chum/issues/141)). +2A/+3 FDC ports are gate-array (not Sinclair ULA-contended); I/O wait is 0.
