...
- **M5 peripherals (in progress)** — joystick modes (Kempston / Sinclair / Cursor), Kempston mouse, Multiface 1 smoke, DivMMC / Interface 1 stubs, Beta Disk VG93 (restore/seek/read/write sector + TR-DOS M1 paging; optional `roms/trdos.rom`), +3 µPD765 command/result on ports `3FFD`/`2FFD` (SEEK/READ/WRITE; see +3DOS below), AY stereo ACB/ABC/Mono UI. Still incomplete vs real hardware: ESXDOS boot, IF1 Microdrive protocol, TR-DOS `RUN"program"` with a real disk ([#138](https://github.com/mward-sudo/spec_chum/issues/138)–[#140](https://github.com/mward-sudo/spec_chum/issues/140)).
